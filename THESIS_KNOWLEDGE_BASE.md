...
| M9A Pilot (Optional) | Domain Adaptation | `openai/clip-vit-base-patch32` | M9A is an optional visual domain-adaptation pilot. It uses duplicate-safe held-out splitting to test whether telecom-specific CLIP/ViT alignment can improve zero-shot visual retrieval. It remains separate from full-corpus M2–M8 ablation. M9A_E0 evaluates zero-shot CLIP on a duplicate-safe held-out test split. Top-100 ranks are stored for inspection, but MRR@10 uses clipped reciprocal-rank contributions only for ranks 1–10. M9A_E1 trains only lightweight projection adapters on frozen CLIP embeddings using the duplicate-safe train split. It is evaluated on the same held-out test split as M9A_E0 and shows higher retrieval metrics than zero-shot CLIP, supporting the value of telecom-specific visual alignment while remaining separate from the full-corpus M2–M8 text-first ablation. |
...
│   ├── 16_m9a_visual_adaptation_planning.py # M9A: Planning and split generation
│   └── 17_m9a_e0_zeroshot_clip_test.py # M9A E0: Zero-shot visual baseline
